...
class ProfileModuleTestCase(unittest.TestCase):
    ...
    def _create_user(self, email_prefix, username_prefix, password, role_mapping):
        unique = uuid.uuid4().hex[:8]

        user = User(
            email=f"{email_prefix}_{unique}@iitm.ac.in",
    ...
    def test_duplicate_profile_prevention(self):
        ...
        response2 = self.client.post(
            ...
            data=json.dumps({
                "full_name": "Applicant Two Duplicate"
            }),
            content_type="application/json"
        )
    ...
    def test_superadmin_access_validation(self):
        profile_mapping = self._create_role_mapping(
            role_name="Applicant",
            module_id="PROFILES"
        )
        ...
        admin_mapping = self._create_role_mapping(
